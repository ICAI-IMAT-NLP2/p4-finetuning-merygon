...
class LoRA(nn.Module):
    def __init__(self, original_layer, r=4, alpha=32):
        ...
        # so that delta_W = A @ B has shape (out_features, in_features)
        self.A = nn.Parameter(torch.empty(out_features, r))
        self.B = nn.Parameter(torch.zeros(r, in_features))

        # Initialize LoRA weights (A with kaiming_uniform, B already zeros)
...
def inject_lora_into_model(model, r=4, alpha=32, device='cpu'):
    ...
    for child_name, child_module in model.named_children():
        ...
        if isinstance(child_module, nn.Linear):
            # Check if this is an attention layer by name - be specific to avoid false matches
            # Only match exact attention layer names (q, k, v, o are single letters for T5)
